...
# Quick Access must not keep an unfinished Passport tile or required callback. It may already be clean from the previous correction.
...
elif "location: 'Cebu City, Central Visayas, Philippines'" not in t:
    raise RuntimeError('registration fixture marker missing')
write(p,t)

# Historical UI tests should assert the current release identity without rewriting their historical scope.
p='tests/build23-ui-foundation.test.ts'
t=read(p)
t=t.replace("expect(index).toContain('Build 25');", "expect(index).toContain('Build 26');")
t=t.replace("expect(startup).toContain('Build 25 could not finish loading.');", "expect(startup).toContain('Build 26 could not finish loading.');")
write(p,t)
...
if "const internalAuth = read('netlify/functions/_lib/internal-auth.mts');" not in t:
...
if old in t:
    ...
    t=t.replace(old,new)
elif t.count('recognition_consent_reference') < 3:
    raise RuntimeError('news RETURNING projections are not hardened')
write(p,t)
...
if "expect(admin).toContain('recognition_consent_at');" not in t:
