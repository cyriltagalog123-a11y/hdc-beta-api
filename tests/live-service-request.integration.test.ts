...
async function api(
  ...
  const response = await handleHdcApiRequest(
    new Request(`https://hdc-build19.test${path}`, {
      ...init,
      headers: {
...
describe.skipIf(!runLiveIntegration)('isolated service-request integration', () => {
  it('registers, signs in, and publishes a request through the production handler', async () => {
    ...
    const email = `hdc-build19-${unique}@example.invalid`;
    const password = 'Build19!Isolated-Test-4829';
    ...
    const registration = await api('/api/auth/register', {
      ...
      body: JSON.stringify({
        email,
        password,
        displayName: 'Build 19 Integration Tester',
        recoveryAnswers: [
    ...
    expect(bootstrap.body.serviceRequests).toEqual(
