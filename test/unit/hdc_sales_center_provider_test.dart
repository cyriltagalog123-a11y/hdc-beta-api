...
Map<String, Object?> _dashboard({
  ...
  'listings': listings ?? [_listing(version: version)],
  'purchaseRequests': [],
};
...
void main() {
  test('Sales Center loads only the bound seller account listings', () async {
    ...
    provider.bindIdentity(
      _identity({HDCPlatformRole.customer, HDCPlatformRole.seller}),
    );
  ...
  test(
    'Sales Center sends integer minor-unit prices and updates its cache',
    () async {
      final store = await _sessionStore();
      ...
      final provider = HdcSalesCenterProvider(
        client: HdcWorkflowApiClient(
          ...
          client: MockClient((request) async {
            ...
            return http.Response(jsonEncode({'listing': _listing()}), 201);
          }),
        ),
      );
  ...
  test(
    'seller acceptance sends a versioned decision and updates orders',
    () async {
      final store = await _sessionStore();
      Map<String, dynamic>? submitted;
      var accepted = false;
        ...
        client: HdcWorkflowApiClient(
          ...
          client: MockClient((request) async {
            if (request.method == 'PUT') {
              ...
              return http.Response(
                jsonEncode({
                  'purchaseRequest': _purchase(status: 'accepted', version: 2),
                }),
                200,
              );
            }
  ...
  test('Sales Center fails closed on a listing from another account', () async {
    final store = await _sessionStore();
    final provider = HdcSalesCenterProvider(
      client: HdcWorkflowApiClient(
        baseUri: Uri.parse('https://example.test'),
        ...
        client: MockClient(
          (_) async => http.Response(
            jsonEncode({
              ...
              'listings': [
                _listing(sellerUserId: '7d67b6b0-97fa-4ac4-9cb3-7f38595576f5'),
              ],
              'purchaseRequests': <Object?>[],
            }),
            200,
    ...
    final provider = HdcSalesCenterProvider(
      client: HdcWorkflowApiClient(
        ...
        client: MockClient((_) async {
          ...
          return http.Response(
            jsonEncode(_dashboard(activeRole: false, listings: [])),
            200,
          );
        }),
      ),
  ...
  test(
    'a listing save rejects duplicates and a pre-save read cannot overwrite it',
    () async {
      final stale = Completer<http.Response>();
      ...
      final provider = HdcSalesCenterProvider(
        client: HdcWorkflowApiClient(
          ...
          client: MockClient((request) async {
            ...
            return reads == 2
                ? stale.future
                : http.Response(jsonEncode(_dashboard()), 200);
          }),
        ),
      );
      provider.bindIdentity(_identity({HDCPlatformRole.seller}));
      ...
      await pumpEventQueue();
      await expectLater(
        provider.createListing({'sellerRole': 'seller'}),
        throwsA(isA<HdcWorkflowException>()),
      );
      saved.complete(
        http.Response(jsonEncode({'listing': _listing(version: 2)}), 201),
      );
      await save;
        ...
        client: MockClient(
          (request) async => http.Response(
            jsonEncode(
              request.method == 'GET'
                  ? _dashboard(listings: [])
                  : {'listing': _listing(sellerUserId: 'different-owner')},
            ),
            200,
          ),
        ),
